"""
Thesis-quality 5-panel reconstruction plot for the 3D (joint) BAE, showing a
single validation wing's reconstruction quality across its span (root to tip)
rather than five different wings at a fixed mid-span slice.

Intended to accompany the per-span reconstruction error plot
(bae3d_error_along_span): it makes concrete what "higher error at root and
tip" actually looks like in the decoded geometry.

All panels share the same y-axis so airfoil proportions are comparable.

Usage
-----
    python -m engiopt.bezier_ae.plot_thesis_bae3d_spanwise_5panel
    python -m engiopt.bezier_ae.plot_thesis_bae3d_spanwise_5panel \
        --checkpoint results/bezier_ae_3d/run_039/models/bezier_ae_3d_best.pt \
        --run_dir    results/bezier_ae_3d/run_039 \
        --wing_index 0 \
        --span_indices 0 3 7 11 14
"""
...
@torch.no_grad()
def plot_5panel(val_dataset, model, device, run_dir, wing_index, span_indices):
    x = val_dataset[wing_index].unsqueeze(0).to(device)   # [1, S, 2, 192]
    S = x.shape[1]
    z = model.encode(x)
    y, cp = model.decode(z, return_cp=True)

    results = []
    for s_idx in span_indices:
        x_s  = x[0, s_idx].cpu()
        y_s  = y[0, s_idx].cpu()
        cp_s = cp[0, s_idx].cpu()
        eta  = s_idx / (S - 1)
    ...
    for idx, ((gt_x, gt_y, recon_x, recon_y, cp_x, cp_y, eta), ax) in \
        ...
        span_label = "Root" if eta == 0.0 else ("Tip" if eta == 1.0 else f"$\\eta={eta:.2f}$")
        ax.text(0.97, 0.97, span_label, transform=ax.transAxes,
    ...
    for ext in ("pdf", "png"):
        path = os.path.join(save_dir, f"bae3d_spanwise_5panel.{ext}")
    ...
    for ext in ("pdf", "png"):
        path = os.path.join(scratch_dir, f"bae3d_spanwise_5panel.{ext}")
        fig.savefig(path, bbox_inches="tight", dpi=150)
        print(f"Saved: {path}")
...
def parse_args():
    ...
    p.add_argument("--wing_index", type=int, default=70)
    p.add_argument("--span_indices", type=int, nargs="+", default=[0, 7, 14])
    return p.parse_args()
...
def main():
    ...
    plot_5panel(val_dataset, model, device, args.run_dir, args.wing_index, args.span_indices)
