"""
DDM_PCA_3D: DDM_PCA using the 3D BezierAutoencoder3D (run_039).

Same idea as DDM_PCA but uses the 3D BAE that encodes the full wing at once
([1, S, 2, 192] → [1, bae_latent_dim]) instead of slice-by-slice.
PCA is fitted on the bae_latent_dim-dimensional vectors directly.
"""
...
class MLPDenoiser(nn.Module):
    """Same architecture as DDM_PCA MLPDenoiser — dedicated AoA head."""
...
class DDM_PCA_3D:
    """Diffusion model in PCA-compressed 3D BAE latent space."""
    ...
    def generate(self, z_init, params, device, T=None):
        ...
        coords : [B, S, 2, 192]
        aoas   : [B]
        z_raw  : [B, z_dim]  (PCA space, denormalised)
        """
        ...
        # PCA inverse → 3D BAE latent [B, bae_latent_dim]
        z_np     = z_raw.cpu().numpy()
        bae_flat = self.pca.inverse_transform(z_np)
        z_bae    = torch.tensor(bae_flat, dtype=torch.float32).to(device)

        # 3D BAE decode: [B, bae_latent_dim] → [B, S, 2, 192]
        self.bae_model.to(device)
        coords = self.bae_model.decode(z_bae).cpu()
        ...
        return coords, aoas.cpu(), z_raw.cpu()
    ...
    def load(self, path, train_mode=True):
        ...
        if not train_mode:
            self.denoiser.eval()
        print(f"Loaded DDM_PCA_3D from {path}")
