...
Extends diffusion_2d_cond to support image conditions (e.g. thermoelastic2d boundary matrices)
alongside scalar conditions. Image conditions are concatenated as extra input channels to the UNet
(standard approach used in Stable Diffusion inpainting, Palette, etc.), while scalar conditions
go through cross-attention.
"""
...
# ---------------------------------------------------------------------------
# Diffusion utilities (identical to diffusion_2d_cond)
# ---------------------------------------------------------------------------
...
class DiffusionSampler:
    ...
    def forward_diffusion_sample_partial(
        ...
        """
        noise = th.zeros_like(x_0).to(device)
    ...
    def sample_timestep(  # noqa: PLR0913
        self,
        ...
        Args:
            model: The UNet diffusion model.
            x: Noisy design tensor of shape ``(B, 1, H, W)``.
            t: Timestep tensor of shape ``(B,)``.
            encoder_hidden_states: Scalar condition tensor for cross-attention.
            img_conds: Optional image conditions of shape ``(B, n_img_conds, H, W)``,
                already resized to the design spatial dimensions. Concatenated as extra
                input channels to the UNet.
            t_mask: Optional mask for the last timestep.
        """
        model.eval()
        with th.no_grad():
            ...
            # Build UNet input by concatenating image conditions as extra channels
            model_input = th.cat([x, img_conds], dim=1) if img_conds is not None else x

            # Call model (current image - noise prediction)
            noise_pred = model(model_input, t, encoder_hidden_states).sample
            ...
            # Noise is sampled with the same shape as x (design only, not model_input)
...
if __name__ == "__main__":
    ...
    # Scalar conditions go through cross-attention; image conditions are input channels
    encoder_hid_dim = n_scalar_conds

    # -----------------------------------------------------------------------
    # Initialize UNet from Huggingface
    # -----------------------------------------------------------------------
    # Image conditions are concatenated as extra input channels to the UNet.
    # When n_img_conds=0, in_channels stays at 1.
    ...
    model = UNet2DConditionModel(
        ...
        encoder_hid_dim=encoder_hid_dim if encoder_hid_dim > 0 else None,
    ...
    # -----------------------------------------------------------------------
    # WandB init (after we know encoder_hid_dim / img info)
    # -----------------------------------------------------------------------
    if args.track:
        extra_config = {
            **vars(args),
    ...
    # Build list of tensors for the dataset: [designs, scalar_cond_0, ..., scalar_cond_N, img_cond_stack]
    tensor_list: list[th.Tensor] = [filtered_ds_norm.flatten(1)]
    tensor_list.extend(training_ds[key][:] for key in scalar_cond_keys)

    # Stack all image conditions into a single (N, n_img_conds, H, W) tensor
    if has_img_conds:
        assert img_cond_shape is not None
        img_cond_tensors = th.stack([training_ds[key][:].float() for key in img_cond_keys], dim=1)
        _expected_ndim = 4  # (N, n_img_conds, H, W)
        if img_cond_tensors.ndim < _expected_ndim:
            img_cond_tensors = img_cond_tensors.unsqueeze(1)
        tensor_list.append(img_cond_tensors)

    training_tensor_ds = th.utils.data.TensorDataset(*tensor_list)

    # Compute scalar condition min/max for sampling visualisation
    cond_tensors = th.stack(training_tensor_ds.tensors[1 : n_scalar_conds + 1])
    ...
    # -----------------------------------------------------------------------
    # Optimizer — UNet parameters only
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Helper: resize image conditions to design spatial dims
    # -----------------------------------------------------------------------
    def resize_img_conds(img_conds: th.Tensor) -> th.Tensor:
        """Resize image conditions to match design spatial dimensions.

        Args:
            img_conds: Image conditions of shape ``(B, n_img_conds, H_cond, W_cond)``.

        Returns:
            Resized tensor of shape ``(B, n_img_conds, design_H, design_W)``.
        """
    ...
    def sample_designs(
        ...
        Args:
            model: The UNet diffusion model.
            n_designs: Number of designs to sample.
            img_conds: Optional image conditions of shape ``(n_designs, n_img_conds, H, W)``.
                Will be resized to design spatial dimensions if not already matching.

        Returns:
            Tuple of (generated images, encoder_hidden_states used for conditioning).
        """
        model.eval()
        with th.no_grad():
            dims = (n_designs, 1, design_shape[0], design_shape[1])
            steps = th.linspace(0, 1, n_designs, device=device).view(n_designs, 1)
            scalar_conds = conds_min.unsqueeze(0) + steps * (conds_max - conds_min).unsqueeze(0)
            # scalar_conds has shape (n_designs, n_scalar_conds)
            encoder_hidden_states = scalar_conds.unsqueeze(1)  # (B, 1, n_scalar_conds)

            # Resize image conditions to design spatial dims if provided
            resized_img: th.Tensor | None = None
            if img_conds is not None:
                resized_img = resize_img_conds(img_conds)

            image = th.randn(dims, device=device)  # only 1 channel for noise
            for i in range(num_timesteps)[::-1]:
                t = th.full((n_designs,), i, device=device, dtype=th.long)
                image = ddm_sampler.sample_timestep(model, image, t, encoder_hidden_states, img_conds=resized_img)
    ...
    for epoch in tqdm.trange(args.n_epochs):
        for i, data in enumerate(dataloader):
            ...
            # Scalar conditions
            scalar_conds = th.stack(data[1 : n_scalar_conds + 1], dim=1)  # (B, n_scalar_conds)
            encoder_hidden_states = scalar_conds.to(device).unsqueeze(1)  # (B, 1, n_scalar_conds)

            # Image conditions (last element if present)
            resized_img: th.Tensor | None = None
            if has_img_conds:
                batch_img_conds = data[n_scalar_conds + 1].to(device)  # (B, n_img_conds, H, W)
                resized_img = resize_img_conds(batch_img_conds)

            current_batch_size = x.shape[0]
            t = th.randint(0, num_timesteps, (current_batch_size,), device=device).long()

            # Get the noise and the noisy input (noise only the design channel)
            x_noisy, noise = ddm_sampler.forward_diffusion_sample(x, t, device)

            # Concatenate image conditions as extra input channels
            unet_input = th.cat([x_noisy, resized_img], dim=1) if resized_img is not None else x_noisy

            noise_pred = model(unet_input, t, encoder_hidden_states).sample
            ...
            if args.track:
                ...
                print(
                    ...
                    f" [loss: {loss.item()}]] [{time.time() - batch_start_time:.2f} sec]"
                )

                # This saves a grid image of 25 generated designs every sample_interval
                if batches_done % args.sample_interval == 0:
                    # For visualisation, grab image conditions from the dataset (first 25) or None
                    vis_img_conds: th.Tensor | None = None
                    if has_img_conds:
                        # Use the first 25 image conditions from the training set (cycled if needed)
                        all_img = training_tensor_ds.tensors[n_scalar_conds + 1]
                        n_avail = all_img.shape[0]
                        indices = th.arange(25) % n_avail
                        vis_img_conds = all_img[indices].to(device)

                    designs_sampled, hidden_states = sample_designs(model, 25, img_conds=vis_img_conds)
                    ...
                    for j, tensor in enumerate(designs_sampled):
                        img = tensor.cpu().numpy()  # Extract x and y coordinates
                        dc = hidden_states[j, 0, :n_scalar_conds].cpu()
